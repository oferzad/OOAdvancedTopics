...
{
    class MonkeyListSolutions
    {
        public List<Monkey> Monkeys { get; private set; }
        public MonkeyListSolutions()
        {
        ...
        {
            #region Without Linq
            //foreach(Monkey m in this.Monkeys)
            //{
            //    if (m.Name == name)
            //        return m;
            //}
            ...
            #endregion
            #region Linq style
            Monkey m = this.Monkeys.Find(x => x.Name == name);
            return m;
            #endregion
        ...
        public List<Monkey> GetAllMonkeysPerLocation(string location)
        {
            ...
            #region Linq Style
            //Linq style:
            //IEnumerable<Monkey> list = this.Monkeys.Where(x => x.Location == location);
            //return list.ToList();
            #endregion
        ...
        //4
        public List<Monkey> SortByLocationAndName()
        {
            #region Linq Style
        ...
        {
            #region Linq Style
            IEnumerable<Monkey> list = from m in this.Monkeys
                                       where m.Name == name
                                       select m;
            Monkey ret = list.FirstOrDefault();
            return ret;
            #endregion
        }
        //6
        ...
        //7
        public List<Monkey> SortByLocationAndNameQuery()
        {
            #region Linq Style
            IEnumerable<Monkey> list = from m in this.Monkeys
        ...
        {
            #region Without Linq
            Dictionary<string, List<Monkey>> d = new Dictionary<string, List<Monkey>>();
            foreach (Monkey m in Monkeys)
            {
                if (d[m.Name] == null)
                    d.Add(m.Name, new List<Monkey>());
                d[m.Name].Add(m);
            }
            foreach (var group in d)
            {
                Console.WriteLine($"Location: {group.Key}, Count: {group.Value.Count}");
                foreach (Monkey m in group.Value)
                    Console.WriteLine(m);
            }
            #endregion
            #region Linq Style
            Dictionary<string, List<Monkey>> dic = this.Monkeys.GroupBy(m => m.Location)
        ...
        {
            #region Linq Style
            Dictionary<string, List<Monkey>> dic = (from m in Monkeys
                                                    group m by m.Location).ToDictionary(d => d.Key, d => d.ToList());
            foreach (var group in dic)
            {
                Console.WriteLine($"Location: {group.Key}, Count: {group.Value.Count}");
                foreach (Monkey m in group.Value)
        ...
        public Monkey[] GetAllMonkeysByName(string name)
        {
            ...
            #endregion
            #region Linq Style
            arr = Monkeys.Where(m => m.Name == name).ToArray();
            #endregion
        ...
        public Dictionary<string, Monkey> CreateDictionaryFromMonkeyList()
        {
            ...
            #endregion
            #region Linq Style
            Dictionary<string, Monkey> dic = this.Monkeys.ToDictionary(m => m.Name);
            return dic;
            #endregion

        }
        //12 - Use the dictionary created in 11 to do the next search!
        public bool MonkyExistByName(string name)
        {
            Dictionary<string, Monkey> dic = CreateDictionaryFromMonkeyList();
            Monkey m = dic[name];
            return (m != null);
        }
    }
